...
class UploadCustomAction extends Action
{
    public $path;
    public $url;
    ...
    public $prefixPath = '';
    public $second_path = null;
    public $second_dimension = [];
    ...
    public function init()
    {
        ...
        if ($this->second_path) {
            $this->second_path = rtrim(Yii::getAlias($this->second_path), DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR;
        }
    }
...
//                );
                ...
                if (!file_exists($this->path) || !is_dir($this->path)) {
                    ...
                    if ($image->save($this->path . $model->{$this->uploadParam}->name, $saveOptions)) {
                        if ($this->second_path) {
                            $image->resize(
                                new Box($this->second_dimension[0], $this->second_dimension[1])
                            );
                            $image->save($this->second_path . $model->{$this->uploadParam}->name, $saveOptions);
                        }
