...
<div class="cropper-widget">
    <?= Html::activeHiddenInput($model, $widget->attribute, ['class' => 'photo-field', 'id' => 'cropper-input-' . $widget->attribute, 'onChange' => $widget->onChange]); ?>
    <label style="cursor: pointer;" class="label" data-toggle="tooltip" title="<?=Yii::t('cropper', 'DEFAULT_LABEL')?>">
        ...
        <?= Html::fileInput('image-input-' . $widget->attribute, null, ['class' => 'sr-only', 'accept' => 'image/*', 'id' => 'image-input-' . $widget->attribute, 'onChange' => $widget->onChange]); ?>
    </label>
